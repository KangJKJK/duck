...
echo -e "${GREEN}Duck_Chain을 설치합니다.${NC}"
...
echo -e "${YELLOW}3. Faucet 자동 실행${NC}"  # 3번 옵션 추가
read -p "선택: " choice

case $choice in
  1)
    echo -e "${GREEN}Duck_Chain을 새로 설치합니다.${NC}"
    ...
    git clone https://github.com/Bachtran301/Duck_Chain
    cd "$WORK"
    ...
    read -p "애플리케이션-세션저장소-Duck_Chain과 관련된 URL클릭 후 나오는 UserID나 QueryID를 적어두세요 (엔터) : "
  ...
  2)
    echo -e "${GREEN}Duck_Chain을 재실행합니다.${NC}"
  ...
  3)  # Faucet 자동 실행 옵션
    echo -e "${GREEN}Faucet 자동 실행을 시작합니다.${NC}"
